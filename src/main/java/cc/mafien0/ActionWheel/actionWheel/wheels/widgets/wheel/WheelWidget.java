...
/**
 * Action Wheel widget Should be set up by using setupSize and setupSlices methods to work properly.
 */
public class WheelWidget implements Renderable, GuiEventListener, NarratableEntry {
  private static final Logger LOGGER = LoggerFactory.getLogger(WheelWidget.class);
  private final LinkedList<WheelWidgetAction> slices = new LinkedList<>();
  public float radius;
  ...
  /**
   * Creates Wheel widget for slice list
   *
   * @param radius Radius of wheel
   * @param x x coordinate of wheel center
   * @param y y coordinate of wheel center
   */
  public WheelWidget setupSize(int radius, int innerRadius, int x, int y) {
    this.radius = radius;
  ...
  public WheelWidget setupSlices(List<? extends WheelAction> slices) {
    this.slices.clear();
    for (var slice : slices) {
      this.slices.add(new WheelWidgetAction(slice, this));
    }
    return this;
  }
  ...
  public boolean mouseClicked(@NonNull MouseButtonEvent event, boolean bool) {
    LOGGER.debug("WheelWidget.mouseClicked: event={}", event);
    boolean cond = false;
