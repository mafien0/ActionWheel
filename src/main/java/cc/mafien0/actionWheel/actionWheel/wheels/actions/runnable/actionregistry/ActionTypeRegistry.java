...
import cc.mafien0.actionWheel.actionWheel.wheels.actions.runnable.actionproviders.*;
...
public class ActionTypeRegistry {
  ...
  public ActionTypeRegistry() {
    ...
    addActionType(_ -> new SubWheelScreenActionProvider(), null);
  ...
  public Gson getGson() {
    var builder =
        new GsonBuilder()
            ...
            .registerTypeAdapter(WheelManager.class, new WheelAdapter());
    typeFactories.forEach(
        e -> {
          if (e.adapter != null) builder.registerTypeAdapter(e.type, e.adapter);
        });
  ...
  public class Adapter
    ...
    @Override
    public ConfigurableRunnable deserialize(
    ...
    public JsonElement serialize(
        ConfigurableRunnable src, Type typeOfSrc, JsonSerializationContext context) {
  ...
  private static class WheelAdapter
      implements JsonSerializer<WheelManager>, JsonDeserializer<WheelManager> {
    @Override
    public WheelManager deserialize(
        JsonElement json, Type typeOfT, JsonDeserializationContext context)
        throws JsonParseException {
      return null;
    }
    ...
    @Override
    public JsonElement serialize(
        WheelManager src, Type typeOfSrc, JsonSerializationContext context) {
