...
import cc.mafien0.actionWheel.actionWheel.wheels.widgets.TextureWidget;
...
public class TextureListAsk extends AskScreen<Identifier> {
  ...
  protected void init() {
    ...
    var accept =
        new Button.Builder(Tr.get("actionWheel.texture.accept"), _ -> success(selectedTexture))
            .bounds(40 + gap, height - 20 - gap, 100, 20)
            .build();
    var cancel =
        new Button.Builder(Tr.get("actionWheel.texture.cancel"), _ -> fail())
  ...
  public void extractRenderState(
    ...
    context.text(
        font,
        Tr.get("actionWheel.texture.selected").append(": ").append(selectedTexture.toString()),
        45,
        height - 30 - 6 - gap / 2,
        0xFFFFFFFF,
        true);
